using System.Data;
using Frameworks.Utilities.SqlUtilities;

namespace BTC_ENTERPRISE.Class
{
    public class KitList
    {
        public static DataSet GetMoh(string mo_number)
        {
        ...
        public static DataSet GetMoDetails(string mo_number)
        {
            DataSet ds = DataSupport.RunDataSet($@"Select mohId[mo_id]
                ,lineNbr [item_number]
                ,item.ref [group]
        ...
        public static DataSet GetMohDetails(string mo_number)
        {
            DataSet dt = DataSupport.RunDataSet($@"SELECT m.[mohId][mo_id]
                ,(Select ecoNum from MIBOMH where bomitem = m.buildItem and bomrev = m.bomrev)[pcn_number]
                ,(Select descr from MIItem where itemid = m.buildItem)[description]
                ,m.[locId][location]
